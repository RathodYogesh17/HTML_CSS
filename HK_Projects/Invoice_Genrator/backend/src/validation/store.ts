...
export const createStoreValidation = Joi.object({
  ...
  panNumber: Joi.string().uppercase().pattern(panRegex).required().messages({
      "string.pattern.base": "Invalid PAN number format"}),
  email: Joi.string().email().lowercase().required(),
  mobile: Joi.string().pattern(mobileRegex).required().messages({
      "string.pattern.base": "Invalid mobile number"}),
  gstType: Joi.string().valid("IGST", "CGST_SGST").default("IGST").required(),
  defaultGstRate: Joi.number().min(0).max(100).default(18).optional()}).options({ abortEarly: false });
...
export const updateStoreValidation = Joi.object({
  ...
  gstNumber: Joi.string().uppercase().pattern(gstRegex).optional(),
  panNumber: Joi.string().uppercase().pattern(panRegex).optional(),
  email: Joi.string().email().lowercase().optional(),
  mobile: Joi.string().pattern(mobileRegex).optional(),
  gstType: Joi.string().valid("IGST", "CGST_SGST").optional(),

...
const Register = () => {
  ...
  // Fix: Properly handle the store data structure
  ...
  // Find selected store name for display
  const getSelectedStoreName = () => {
    if (!selectedStore) return "No store selected";
    const store = stores.find((s: Store) => s._id === selectedStore);
              ...
              <div className="user-store-container">
                ...
                >
                  {stores.length > 0 ? (
                    ...
                    <Option disabled value="no-stores">No stores available</Option>
                  )}
                </Select>
